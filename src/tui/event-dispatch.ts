...
export async function dispatchEvent(event: HarnessEvent, ectx: EventHandlerContext, state: TUIState): Promise<void> {
  switch (event.type) {
    ...
    case 'thread_changed': {
      ...
      const threads = await listThreads(state);
      ...
      if (currentThread) {
        ...
        if ('activeGithubPrSubscriptions' in state) {
    ...
    case 'task_updated': {
      ...
      if (state.taskProgress) {
        ...
        const previousTasks = (getDisplayStateSnapshot(state).previousTasks ?? []) as TaskItemSnapshot[];
    ...
    case 'tool_suspended': {
      ...
      if (event.toolName === 'request_access' || payload.kind === 'sandbox_access_request') {
        ...
        await handleAskQuestion(
          ...
          payload.selectionMode as AskQuestionSelectionMode | undefined,
        );
