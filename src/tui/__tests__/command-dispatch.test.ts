...
const mocks = vi.hoisted(() => ({
  ...
  handlePentestCommand: vi.fn().mockResolvedValue(undefined),
...
vi.mock('../commands/index.js', () => ({
  ...
  handlePentestCommand: mocks.handlePentestCommand,
}));
...
describe('dispatchSlashCommand models routing', () => {
  beforeEach(() => {
    ...
    mocks.handlePentestCommand.mockClear();
  ...
  it('routes /pentest to handlePentestCommand', async () => {
    const state = {
      customSlashCommands: [],
      harness: {
        getCurrentThreadId: vi.fn(() => 'thread-1'),
    ...
    const handled = await dispatchSlashCommand('/pentest http://34.166.198.47:40023/login blind SSTI', state, () => ctx);

    expect(handled).toBe(true);
    expect(mocks.handlePentestCommand).toHaveBeenCalledTimes(1);
    expect(mocks.handlePentestCommand).toHaveBeenCalledWith(ctx, [
      'http://34.166.198.47:40023/login',
      'blind',
      'SSTI',
    ]);
    expect(mocks.showError).not.toHaveBeenCalled();
    expect(mocks.trackCommand).toHaveBeenCalledWith('pentest', {
      action: 'attempted',
      threadId: 'thread-1',
      resourceId: 'resource-1',
      mode: 'build',
