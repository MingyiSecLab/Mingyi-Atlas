...
import { GATEWAY_AUTH_HEADER, MastraGateway, MastraModelGateway, ModelRouterLanguageModel } from '@mastra/core/llm';
...
function resolveDirectModel(
  ...
  if (normalizedModelId.startsWith('github-copilot/')) {
    return githubCopilotProvider(normalizedModelId.substring('github-copilot/'.length), { headers });
  }
...
function createMastraCodeGateway({
  ...
  return new (class MastraCodeGateway extends MastraModelGateway {
    ...
    resolveAuth(request: GatewayAuthRequest) {
      ...
      const customProvider = customProviders.find(provider => request.providerId === getCustomProviderId(provider.name));
...
export function resolveModel(
  ...
  const resolvedRouterModelId = modelId;
  ...
  if (!isMastraGatewayModel) {
    return resolveDirectModel(normalizedModelId, headers, options?.thinkingLevel, settings.customProviders);
  }
