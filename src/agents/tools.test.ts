...
vi.mock('../tools/pentest/index.js', () => ({
  createPentestTools: () => ({
    run_pentest_workflow: { description: 'run pentest workflow' },
    discover_attack_surface: { description: 'discover attack surface' },
  }),
}));
...
describe('createDynamicTools', () => {
  ...
  it('exposes pentest tools only in pentest mode', () => {
    const getDynamicTools = createDynamicTools();
    ...
    expect(buildTools.run_pentest_workflow).toBeUndefined();
    expect(pentestTools.run_pentest_workflow).toBeDefined();
    expect(pentestTools.discover_attack_surface).toBeDefined();
  });
