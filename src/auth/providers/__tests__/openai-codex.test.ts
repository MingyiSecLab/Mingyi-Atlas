...
describe('OpenAI Codex OAuth callback port selection', () => {
  ...
  it('uses mingyi-atlas as the default OAuth originator', async () => {
    const { __testing } = await import('../openai-codex.js');

    const { url } = await __testing.createAuthorizationFlow('http://localhost:1455/auth/callback', 'state');

    expect(new URL(url).searchParams.get('originator')).toBe('mingyi-atlas');
  });
...
describe('openaiCodexOAuthProvider auth modes', () => {
  const originalAuthMode = process.env.MINGYI_ATLAS_OPENAI_CODEX_AUTH_MODE;

  afterEach(() => {
    vi.unstubAllGlobals();
    if (originalAuthMode === undefined) {
      delete process.env.MINGYI_ATLAS_OPENAI_CODEX_AUTH_MODE;
    } else {
      process.env.MINGYI_ATLAS_OPENAI_CODEX_AUTH_MODE = originalAuthMode;
    }
  });
  ...
  it('honors callbacks.authMode="browser" even when the env var requests device mode', async () => {
    process.env.MINGYI_ATLAS_OPENAI_CODEX_AUTH_MODE = 'device';
