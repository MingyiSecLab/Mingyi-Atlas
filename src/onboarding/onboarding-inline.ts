...
export class OnboardingInlineComponent extends Box implements Focusable {
  ...
  private updateModePackDetail(packs: ModePack[], highlightedId: string): void {
    ...
    if (pack.id === 'custom') {
      ...
      const detail = [
        ...
        `  ${chalk.hex(mastra.orange)('pentest')} → ${theme.fg('text', pack.models.pentest)}`,
      ].join('\n');
  ...
  private async runCustomPackFlow(): Promise<void> {
    ...
    this.collapseStep(
      `Model pack → ${theme.bold(packName)}  ` +
        ...
        `${chalk.hex(mastra.orange)('pentest')} ${models.pentest}`,
    );
    this.renderStep('omPack');
    this.tui.requestRender();
  }
  ...
  private renderDone(): void {
    ...
    const lines = [
      ...
      `  ${chalk.hex(mastra.orange)('pentest')} → ${this.selectedModePack.models.pentest}`,
