...
describe('createBrowserFromSettings — recording tools gating', () => {
  ...
  it.each([
    ...
    'exposes recording tools on a Mingyi Atlas-constructed %s browser while keeping provider tools intact',
    async (provider, providerToolName) => {
      ...
      expect(tools[providerToolName], `expected tool ${providerToolName} to be present`).toBeDefined();
    },
  );
