...
import {
  createPentestMode,
  createPentestRuntimeProcessorProvider,
  getPentestRuntimeSubagents,
  getPentestSubagentModeMap,
} from './agents/pentest/runtime/index.js';
...
export async function createMastraCode(config?: MastraCodeConfig) {
  ...
  // Agent
  const getPentestRuntimeInputProcessors = createPentestRuntimeProcessorProvider();
    ...
    inputProcessors: ({ requestContext }) => {
      ...
      const processors: InputProcessorOrWorkflow[] = [
        new AgentsMDInjector({
          getIgnoredInstructionPaths: () => getStaticallyLoadedInstructionPaths(projectPath),
        }),
        new ProviderHistoryCompat(),
      ];

      processors.push(...getPentestRuntimeInputProcessors(harnessContext));
  ...
  const defaultSubagents = [exploreSubagent, planSubagent, executeSubagent, ...getPentestRuntimeSubagents()];
  ...
  const defaultModes: HarnessMode<Record<string, unknown>>[] = [
    ...
    createPentestMode(codeAgent, mastra.red),
  ];
