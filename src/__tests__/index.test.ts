...
describe('createMastraCode', () => {
  ...
  it('registers pentest as an additive default mode', async () => {
    const { createMastraCode } = await import('../index.js');
    ...
    const harnessConfig = harnessConstructorMock.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(harnessConfig?.modes?.map(mode => mode.id)).toEqual(['build', 'plan', 'fast', 'pentest']);
    expect(harnessConfig?.modes?.find(mode => mode.id === 'pentest')?.name).toBe('Pentest');
  });
  ...
  it('configures ProviderHistoryCompat for prompt and API error compatibility', async () => {
    ...
    const agentConfig = agentConstructorMock.mock.calls[0]?.[0] as
      | {
          inputProcessors?: ((args: { requestContext: { get: (key: string) => unknown } }) => Array<{ id?: string }>);
          errorProcessors?: Array<{ id?: string }>;
        }
      | undefined;
    const inputProcessors = agentConfig?.inputProcessors?.({
      requestContext: {
        get: key => (key === 'harness' ? { modeId: 'build', getState: () => ({ projectPath: process.cwd() }) } : undefined),
      },
    });
  ...
  it('adds on-demand skill discovery only for pentest mode when a workspace is available', async () => {
    const { createMastraCode } = await import('../index.js');
    ...
    const buildProcessors = agentConfig?.inputProcessors?.({
      requestContext: {
        get: key =>
          key === 'harness'
            ? { modeId: 'build', workspace, getState: () => ({ projectPath: process.cwd() }) }
            : undefined,
      },
    });
    const pentestProcessors = agentConfig?.inputProcessors?.({
      requestContext: {
        get: key =>
          key === 'harness'
            ? { modeId: 'pentest', workspace, getState: () => ({ projectPath: process.cwd() }) }
            : undefined,
      },
    });

    expect(buildProcessors?.map(processor => processor.id)).not.toContain('skill-search');
    expect(pentestProcessors?.map(processor => processor.id)).toContain('skill-search');
  });

  it('does not add on-demand skill discovery for pentest mode without a workspace', async () => {
    const { createMastraCode } = await import('../index.js');
    ...
    const pentestProcessors = agentConfig?.inputProcessors?.({
      requestContext: {
        get: key => (key === 'harness' ? { modeId: 'pentest', getState: () => ({ projectPath: process.cwd() }) } : undefined),
      },
    });

    expect(pentestProcessors?.map(processor => processor.id)).not.toContain('skill-search');
  });
});
